...
GUI 使用扁平的隧道表，每行自行指定 `tows`、目标和本地监听。启动时按规范化后的 `tows` 地址分组：上例建立两条 WebSocket，每条承载对应服务器的 SSH 和 Minecraft 隧道。这样不会为每条规则重复建立连接，也不会让一台服务的网络故障中断另一台服务。

可把一个或多个 `.json` 文件、或包含配置的文件夹拖入窗口。导入前可选择跳过同名、覆盖同名或整体替换。来源文件永远不会被改写。缺少名称的隧道会得到基于 `tows`、目标和监听地址生成的确定性名称；重复名称、非法端口和本地监听冲突会被拒绝或高亮。
