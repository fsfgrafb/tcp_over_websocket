...
使用 `towc --version` 或 `tows --version` 查看程序版本。
...
- 短信登录：输入手机号，收到验证码后在终端或 GUI 输入；
- 邮箱验证码：输入邮箱，收到验证码后在终端或 GUI 输入。

认证缓存只保存 WebVPN ticket，不保存用于短信登录的手机号、邮箱、验证码、二维码回调 code 或二维码图片。缓存和配置位于：
...
Cookie、交互默认值、GUI 配置、GUI 状态和程序日志均保存在上述同一目录。`towc.log` 与 `tows.log` 采用追加写入，单文件最多 2 MiB；达到上限后保留最新日志。

每条 WebSocket 默认每 60 秒发送一次 WebSocket Ping，并要求在下一个周期前收到 Pong；同一次客户端会话每 10 分钟只全局刷新一次 WebVPN Cookie。`towc_gui` 中某条 WebSocket 断开时，只停止属于该 `tows` 的本地监听，其他服务器组继续工作；全局 Cookie 刷新失败时才停止全部服务器组。微信二维码过期后会自动获取新二维码；也可切换到短信或邮箱登录，无需重启程序。

GUI 隧道状态只使用颜色圆点表示：绿色表示隧道可用，黄色表示正在连接或监听范围需要注意，红色表示配置或运行异常，灰色表示隧道已禁用。将鼠标停留在圆点上可查看底层状态详情。

控制台日志正文统一使用 ASCII 英文，避免不支持中文的终端出现乱码。每行日志以彩色组件标签开头：`[towc]` 表示客户端生命周期与认证，隧道日志直接使用 `[隧道名称]`，`[tows]` 表示服务端生命周期；GUI 日志面板使用相同标签但不包含 ANSI 转义序列。
...
{
  "connections": [
    { "tows": "10.18.47.77:4489", "ws_keepalive_secs": 60 },
    { "tows": "10.18.47.66:4489", "ws_keepalive_secs": 60 }
  ],
  "tunnels": [
...
GUI 按规范化后的 `tows` 地址分组展示隧道，每行自行指定目标和本地监听。连接可以先于隧道单独创建；没有启用隧道时不建立 WS。某连接的第一条隧道启用时建立一条共享 WS，最后一条关闭时停止该 WS，其他服务器连接不受影响。每个连接可单独设置保活间隔，底层通过 WS PING 实现。

程序启动后自动检查 Cookie：有效时进入认证完成状态；无效时自动开始微信登录。即使配置为空，认证任务仍会按全局可配置间隔调用 Cookie 更新接口，但不会建立 WS。研究实测 Cookie 刷新后的有效窗口约为 880–920 秒，因此 GUI 将 Cookie 刷新间隔限制为 60–840 秒；设置中留空时使用默认值 600 秒。二维码过期后可原地重新生成，也可在微信、短信和邮箱登录之间切换。
...
每行最前面的方形选择框与启用开关互不相关；选择状态不持久化，并在触发导出后自动清空。主题和 Cookie 刷新间隔保存在独立的 `gui-state.json`。“导出选中隧道”打开系统另存为窗口，默认目录为用户桌面，并把全部选中项写入一个 JSON 文件。
...
来源 IP 变化或 ticket 过期会使 WebVPN 拒绝连接。CLI 重新运行后会先验证缓存；GUI 可直接重新生成微信二维码或改用短信/邮箱登录。

## 开发

Rust 1.95+，edition 2024。常用命令：
