...
每条 WebSocket 每 60 秒独立发送一条协议 PING；同一次客户端会话每 10 分钟只全局刷新一次 WebVPN Cookie。`towc_gui` 中某条 WebSocket 断开时，只停止属于该 `tows` 的本地监听，其他服务器组继续工作；全局 Cookie 刷新失败时才停止全部服务器组。微信二维码过期后会自动获取新二维码；也可切换到短信或邮箱登录，无需重启程序。
