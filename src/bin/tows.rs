...
use tcp_over_websocket::{
    SERVER_LISTEN_ADDR, SERVER_LISTEN_HOST, accept_websocket_with_path, log_error, log_info,
...
async fn handle_connection(stream: TcpStream) -> Result<()> {
    ...
    let (websocket, path) = accept_websocket_with_path(stream).await?;
    let target_addr = parse_tcp_target_path(&path)?;
    let target = TcpStream::connect(&target_addr)
        .await
        .with_context(|| format!("failed to connect target tcp endpoint: {target_addr}"))?;
