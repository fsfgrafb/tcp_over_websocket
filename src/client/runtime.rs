...
use crate::network::{
    HEARTBEAT_INTERVAL, build_webvpn_ws_url, client_handshake, connect_websocket, is_cookie_expired,
};
use crate::protocol::{Frame, FrameType, MAX_DATA_LEN, MAX_TUNNELS};
use crate::{APP_VERSION, init_tracing};

use super::auth::{
    AuthPrompt, AuthenticationExpired, SessionCookie, login_or_restore, login_with_preference,
    refresh_ticket, restore_valid_cached_ticket,
};
...
pub(crate) fn authentication_expired(error: &anyhow::Error) -> bool {
    error.downcast_ref::<AuthenticationExpired>().is_some() || is_cookie_expired(error)
}
                    ...
                    Ok((server, generation, result)) => {
                        ...
                        if let Err(error) = result {
                            ...
                            if authentication_expired(&error) {
                                ...
                                while tasks.join_next().await.is_some() {}
                                return Err(error.context("WebVPN authentication expired"));
                            }
                        }
        ...
        tokio::select! {
            ...
            message = source.next() => {
                match message {
                    ...
                    Some(Err(error)) => break Err(anyhow!(error).context("WebSocket read failed")),
                    None => break Err(anyhow!("WebSocket disconnected")),
                }
            }
            event = event_rx.recv() => {
...
mod tests {
    use super::*;
    use crate::network::ConnectFailure;
    ...
    #[test]
    fn only_explicit_authentication_failures_request_a_new_login() {
        let redirect = anyhow!(ConnectFailure::CookieExpired {
            location: "/webvpn.szut.edu.cn/login".to_string(),
        })
        .context("connecting to tows");
        assert!(authentication_expired(&redirect));

        let refresh = anyhow!(AuthenticationExpired).context("refreshing cookie");
        assert!(authentication_expired(&refresh));

        let network_change = anyhow!("peer closed connection without TLS close_notify")
            .context("WebSocket read failed");
        assert!(!authentication_expired(&network_change));
    }
