...
impl fmt::Display for AuthenticationExpired {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("WebVPN login is no longer valid; sign in again")
    }
}

impl std::error::Error for AuthenticationExpired {}
...
pub(crate) async fn refresh_ticket(cookie: &SessionCookie) -> Result<()> {
    if !validate_protected_session(cookie).await {
        return Err(AuthenticationExpired.into());
    }
    let jar = Arc::new(reqwest::cookie::Jar::default());
...
mod tests {
    ...
    #[test]
    fn response_redaction_preserves_utf8_boundaries() {
        let body = format!("{}中secret", "a".repeat(119));
        let redacted = redact_code(&body);
