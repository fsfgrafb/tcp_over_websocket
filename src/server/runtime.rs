...
const MAX_TARGET_CONNECTIONS: usize = 1024;
...
pub async fn serve(listener: TcpListener, mut stop: watch::Receiver<bool>) -> Result<()> {
    ...
    let target_slots = Arc::new(Semaphore::new(MAX_TARGET_CONNECTIONS));
    loop {
        tokio::select! {
            ...
            accepted = listener.accept() => {
                ...
                let target_slots = Arc::clone(&target_slots);
                tokio::spawn(async move {
                    let _slot = slot;
                    if let Err(error) = handle_connection(stream, peer, target_slots).await {
...
struct OpenTunnel {
    ...
    _target_slot: OwnedSemaphorePermit,
}
...
async fn handle_ws_message(
    ...
    match message {
        Message::Binary(bytes) => {
            ...
            handle_frame(frame, writer, events, tunnels, target_slots).await
        }
...
async fn handle_frame(
    ...
    match frame.kind {
        FrameType::Open => {
            ...
            let Ok(target_slot) = Arc::clone(target_slots).try_acquire_owned() else {
                let reason = "tows has reached the global target connection limit"
                    .as_bytes()
                    .to_vec();
                writer
                    .send(Frame::new(FrameType::OpenFail, frame.tunnel_id, reason)?)
            ...
            tunnels.insert(frame.tunnel_id, Tunnel::Opening(target_slot));
...
async fn handle_tunnel_event(
    ...
    match event {
        TunnelEvent::Connected(id, result) => {
            let Some(Tunnel::Opening(target_slot)) = tunnels.remove(&id) else {
                return Ok(());
            };
            match result {
                Ok(stream) => {
                    ...
                    writer
                        ...
                        .await?;
                    let tunnel = spawn_tcp_tasks(id, stream, flow, events.clone(), target_slot);
...
fn spawn_tcp_tasks(
    ...
    flow: FlowWriter,
    events: mpsc::Sender<TunnelEvent>,
    target_slot: OwnedSemaphorePermit,
) -> OpenTunnel {
...
fn open_tunnel_mut(tunnels: &mut HashMap<u16, Tunnel>, id: u16) -> Result<&mut OpenTunnel> {
    match tunnels.get_mut(&id) {
        ...
        Some(Tunnel::Opening(_)) => bail!("tunnel {id} received data before OPEN_OK"),
