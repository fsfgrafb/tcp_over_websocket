...
#[cfg(feature = "client")]
use tokio_tungstenite::{MaybeTlsStream, connect_async_with_config};
use tokio_tungstenite::{WebSocketStream, accept_hdr_async_with_config};

#[cfg(feature = "client")]
use crate::address::Endpoint;
use crate::protocol::{Frame, HEADER_LEN, MAX_PAYLOAD_LEN, PROTOCOL_VERSION};
...
const MAX_WEBSOCKET_MESSAGE_LEN: usize = HEADER_LEN + MAX_PAYLOAD_LEN;
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::CookieExpired { location } => write!(
                formatter,
                "WebVPN login has expired; sign in again (location: {location})"
            ),
...
#[cfg(feature = "client")]
pub(crate) fn is_cookie_expired(error: &anyhow::Error) -> bool {
    error
        .downcast_ref::<ConnectFailure>()
        .is_some_and(|failure| matches!(failure, ConnectFailure::CookieExpired { .. }))
}
...
fn encrypt_webvpn_host(host: &str) -> Result<String> {
    ...
    for chunk in host.as_bytes().chunks(16) {
        let mut block = Array::from(feedback);
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let parsed = (|| -> Result<(u16, String)> {
        ...
        Ok((version, server_program.to_string()))
    })();
    let (version, server_program) = match parsed {
        Ok(parsed) => parsed,
        Err(error) => {
            close_handshake_protocol(websocket, &error.to_string()).await;
            return Err(error);
        }
    };
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let parsed = (|| -> Result<(u16, String)> {
        ...
        Ok((version, client_program.to_string()))
    })();
    let (version, client_program) = match parsed {
        Ok(parsed) => parsed,
        Err(error) => {
            close_handshake_protocol(websocket, &error.to_string()).await;
            return Err(error);
...
pub async fn accept_websocket(stream: TcpStream) -> Result<(WebSocketStream<TcpStream>, String)> {
    ...
    let websocket = accept_hdr_async_with_config(
        stream,
        move |request: &ServerRequest, response| {
            let requested = request.uri().path().to_string();
            *captured.lock().expect("path mutex poisoned") = Some(requested.clone());
            if requested != "/" {
                return Err(tokio_tungstenite::tungstenite::http::Response::builder()
                    .status(tokio_tungstenite::tungstenite::http::StatusCode::NOT_FOUND)
                    .body(Some("WebSocket endpoint is /".to_string()))
...
fn websocket_config() -> WebSocketConfig {
    WebSocketConfig::default()
        .max_message_size(Some(MAX_WEBSOCKET_MESSAGE_LEN))
        .max_frame_size(Some(MAX_WEBSOCKET_MESSAGE_LEN))
}

async fn close_handshake_protocol<S>(websocket: &mut WebSocketStream<S>, reason: &str)
where
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let _ = websocket
        .send(Message::Close(Some(CloseFrame {
            ...
            reason: Utf8Bytes::from(reason[..end].to_string()),
        })))
        .await;
}
