...
use crate::client::{
    AuthPrompt, ClientObserver, ForwardRule, LoginPreference, ServerGroup, authentication_expired,
    clear_cached_ticket, login_or_restore, run_dynamic_server_groups,
};
...
impl TowcApp {
    ...
    fn start(&mut self) {
        ...
        std::thread::spawn(move || {
            ...
            let authentication_expired = result.as_ref().is_err_and(authentication_expired);
            let _ = event_tx.send(WorkerEvent::Finished {
                result: result.map_err(|error: anyhow::Error| format!("{error:#}")),
                ...
                WorkerEvent::Finished {
                    ...
                    if self.logout_when_stopped {
                        ...
                        match result {
                            ...
                            Err(error) => {
                                self.status = if authentication_expired {
                                    "WebVPN 登录已过期，正在重新登录".to_string()
                                } else {
                                    "连接已停止".to_string()
                                };
                                self.login_visible = true;
                                ...
                                if authentication_expired {
                                    if let Err(clear_error) = clear_cached_ticket() {
                                        self.log(format!(
                                            "无法删除已过期的登录凭据，将直接重新认证：{clear_error:#}"
                                        ));
                                    }
                                    self.log("WebVPN 登录已过期，正在重新认证".to_string());
                                    self.auto_start_pending = true;
                                }
                            }
                        }
...
impl eframe::App for TowcApp {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ui, |ui| {
            egui::Panel::bottom("fixed-bottom-actions")
                .exact_size(52.0)
                .resizable(false)
