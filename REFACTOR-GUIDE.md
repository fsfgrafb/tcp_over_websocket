...
1. **tows 地址**：读交互缓存 `interactive.defaults`
   - 有缓存 → `tows address <ip[:port]> (default: X): `，回车复用
   - 无缓存 → `tows address <ip[:port]>: `，必填（空/非法 → 重新提示）
...
- 三程序共享同一版本号，**起步版本 v0.5.0**（2026-08-06 用户确认；内网版本分布：**仅 10.18.47.77 装 v0.5.0，其余内网机器为 v0.3/v0.4**——v0.5.0 即内网最新编号，重构延续推进该编号，虽协议不兼容但版本号向前走），一起发布（GitHub Release 打包 zip）
- 交叉编译目标：
