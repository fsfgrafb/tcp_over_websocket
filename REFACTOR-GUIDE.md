...
- **客户端**：收到 HELLO_ACK 且协议版本一致 → 正常复用；收到 `"连接成功"` 文本（仅旧 tows 会发）/ 超时 / 版本不一致 → **打印警告并退出**（提示升级 tows），**不降级、不重试**
...
### 3.3 路径解析模式（已废弃，不兼容旧版）
- `/tcp/{host}:{port}` 路径解析模式**不再支持**（v0.4/acca315 遗留），tows 只接受多路复用协议（§3.0）
- 旧版行为细节（`"连接成功"` 的回显 vs 主动发送差异）见 §4 历史实现，仅作参考
...
- **已补充（2026-08-06）**：
  ...
  - **版本协商（2026-08-06）**：客户端建 WS 第一帧发 HELLO(0x00) → tows 回 HELLO_ACK(0x07)；**不兼容旧版**：发现旧 tows 打印警告并退出（towc/towc_gui 均不降级），tows 拒绝旧客户端；towc 统一走多路复用协议（单隧道=1 tunnel_id），路径解析模式已废弃（§3.0/§3.1）
