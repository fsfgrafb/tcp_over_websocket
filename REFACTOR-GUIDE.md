...
### 保活架构设计（2026-08-05 用户确认；towc 参数可配置，⚠️ 默认值为临时设定）
- **每个 towc = 1 个进程 = 1 条 WS 连接**（逻辑代理转发隧道），该连接同时承载数据与 **60s 心跳** → 网关视角所有到 tows 的连接始终活跃，无空闲超时风险；无独立保活 WS、无静默连接概念
- **多隧道 = 多 towc 进程**（各自独立一条连接；towc_gui 可启动多个子进程管理）
- **Cookie 刷新 = 全局一个，间隔默认 10 分钟（600s）**：所有 towc 共享同一 cookie
  - towc_gui 场景：GUI 统一刷新并分发给子进程
  - 独立 towc：各自刷新（重复无害）或文件锁选主
- **错误处理：towc 不做 IP 变化检测、不自动重登**——IP 变化 / cookie 过期等一切失效都通过保活/连接反馈直接体现（302 `/login?logoutByIpChange=true`、WS 握手被拒、保活失败），towc 检测到任一失败即**直接退出**并提示用户重新启动程序重新登录；不引入"检测-重登"状态机，保持简单可靠
...
- **已补充（2026-08-05）**：
  ...
  - 保活架构：1 towc = 1 连接（数据+60s 心跳合体）、全局 cookie 刷新 10min（§2.5）
  - 错误处理：不检测 IP、不自动重登，失败即退出（§2.5）
