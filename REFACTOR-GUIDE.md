...
- **保活配置（towc，参数可配置；⚠️ 默认值为临时设定，可能修改）**：
  - **cookie 保活间隔：默认 10 分钟（600s）**——全局仅需一个 towc 刷新即可
  - **连接心跳：默认 60 秒**——每条 towc↔tows 连接（数据隧道与保活合体）每 60s 发一次心跳，无静默连接

### 保活架构设计（2026-08-05 用户确认）
- **每个 towc = 1 个进程 = 1 条 WS 连接**（逻辑代理转发隧道），该连接同时承载数据与 60s 心跳 → 网关视角所有到 tows 的连接始终活跃，无空闲超时风险
- **多隧道 = 多 towc 进程**（各自独立一条连接；towc_gui 可启动多个子进程管理）
- **Cookie 刷新 = 全局一个**：所有 towc 共享同一 cookie，每 10 分钟只需一个刷新
  - towc_gui 场景：GUI 统一刷新并分发给子进程
  - 独立 towc：各自刷新（重复无害）或文件锁选主
- 无独立保活 WS、无静默连接概念
- **错误处理（2026-08-05 用户确认）：towc 不做 IP 变化检测、不自动重登**
  - IP 变化 / cookie 过期等一切失效，都会通过保活/连接反馈直接体现（如 302 `/login?logoutByIpChange=true`、WS 握手被拒、保活失败）
  - towc 检测到上述任一失败 → **直接退出**，提示用户重新启动程序重新登录即可
  - 不引入复杂的"检测-重登"状态机；保持简单可靠，失败即退出
...
### 3.2 数据与控制帧
- **二进制帧** = TCP 数据（双向）
- **文本 `"连接成功"`** = 心跳（客户端默认每 60s 发，**可配置**；服务端回显；客户端收到就忽略）
...
- **v0.4.0** 有验证码登录（`login_with_verification_code`）：接口/execution/RSA倒序/重试均与实测一致 ✅；
  ⚠️ **提交字段缺 `rememberMe`**——实测手机登录必须带它 → v0.4 手机验证码登录可能存在 bug
