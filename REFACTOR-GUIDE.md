...
- ⚠️ **待实测**：单 WS 多路复用时所有隧道共享单连接带宽，需实测多隧道并发总吞吐是否够用（确认网关是"单连接限速 ~6.1MB/s"还是"全局不限"）
...
3. tows 连目标 TCP 成功后发送文本 `"连接成功"` 作为就绪确认
...
- 实测基线（目标内网 10.18.47.77，网关为中转）：
  ...
  - 隧道吞吐 ≈ 6.10 MB/s（约 48Mbps，**单连接网关上限**，非本机瓶颈）
