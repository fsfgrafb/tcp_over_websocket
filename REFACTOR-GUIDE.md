...
- **tows**（控制台，跨平台）：跑在能访问目标服务的内网机器上。监听固定端口（**可选启动参数** `tows [port]`，默认 4489）、接受 WebSocket、路由到目标 TCP、双向转发。
  ...
  - 兼容两种模式：① 多路复用协议（新，towc_gui 用，见 §3.1/3.2）；② 路径解析 `/tcp/{host}:{port}`（旧，兼容 towc 单隧道，见 §3.3）
...
- **towc（单隧道控制台）**：仍为 1 进程 = 1 条 WS = 1 隧道，心跳 60s；**与 towc_gui 统一走多路复用协议**（单隧道 = 1 个 tunnel_id，§3.1/3.2），与 towc_gui **共享同一套协议层**；路径解析模式仅用于**兼容旧 v0.4/acca315 客户端**（§3.3）
...
### 3.0 两种模式 + 版本协商（tows 兼容）
- **多路复用模式（新，towc/towc_gui 统一使用）**：1 条 WS 连接承载多个隧道（towc 单隧道 = 1 个 tunnel_id），帧头带 tunnel_id，tows 按包路由（见 §3.1/3.2）
- **路径解析模式（旧，仅兼容 v0.4/acca315 老客户端）**：从 WS 请求路径解析目标（见 §3.3）
- **版本协商（2026-08-06 用户确认）**：客户端建 WS 后**第一帧发 HELLO**（0x00，带协议版本+程序版本）→ tows 回 HELLO_ACK（0x07）→ 进入复用模式
- tows 模式判断（连接建立后）：
  - 收到 HELLO 帧 → 复用模式，回 HELLO_ACK
  - 收到**非 HELLO 二进制帧**（老客户端直接发数据）→ 路径解析模式（无需等待）
  - **3s 超时无任何帧**（老 acca315 towc 在等就绪确认不发数据）→ 降级路径解析模式
- ⚠️ 版本不匹配（客户端侧）：收到文本 `"连接成功"`（而非 HELLO_ACK）→ **tows 是旧版** → towc 打印警告并降级路径解析继续 / towc_gui 警告并退出；超时 → 按旧版处理并提示
...
- **协议版本常量：PROTOCOL_VERSION = 2**（v1 = 旧路径解析无握手）；HELLO 中协议版本不匹配 → 客户端提示并降级/退出
...
### 3.3 路径解析模式（仅兼容旧 v0.4/acca315 客户端；新版 towc/towc_gui 不走此模式）
1. towc 用 `wss://webvpn.szut.edu.cn/ws-{tows端口}/{编码(tows所在主机)}/{目标路径}` 建 WS（请求头带 `Cookie: wengine_vpn_ticket...=...`）
2. tows 从**请求路径**解析目标：
   - `/tcp` → 默认 `127.0.0.1:22`
   - `/tcp/{port}` → `127.0.0.1:{port}`
   - `/tcp/{host}:{port}` → 指定 host
3. tows 连目标 TCP 成功后发送文本 `"连接成功"` 作为就绪确认（**新版 tows 降级路径解析时沿用 acca315 行为：主动发送**）
4. 双向转发开始；文本 `"连接成功"` 兼作心跳（60s，服务端回显）；文本 `"tows-tcp-eof"` = TCP EOF（对端应 shutdown 另一方向）
5. ⚠️ **历史行为差异（代码实测确认）**：
   - **v0.4**：`"连接成功"` 仅作**心跳回显**——towc 主动发、tows 原样回显（`WebVpnHeartbeatRole::Server.echoes_heartbeat()`），tows **不主动发送**
   - **acca315（新版）**：tows 连目标成功后**主动发送** `"连接成功"`（`TOWS_READY_MESSAGE = WEBVPN_HEARTBEAT_MESSAGE`，server/runtime.rs）作就绪确认
...
3. **三个可执行文件**：tows（可选启动参数，**须支持多路复用路由**）+ towc（**带参模式 + 无参交互模式**，v0.4 兼容）+ towc_gui（**Windows 专属 GUI，单进程多隧道，自包含不依赖 towc.exe**）
4. **登录三种方式**：微信扫码 / 手机验证码 / 邮箱验证码（见 §2.6/2.7）
5. **多路复用协议 + 版本协商**：towc/towc_gui 与 tows 之间单 WS 多路复用（见 §3.1/3.2），**建连第一帧发 HELLO 握手**（0x00/0x07，§3.0），帧格式以简洁为准
...
- **已补充（2026-08-05）**：
  ...
  - **架构选定：方案 2（单进程多隧道）**——towc_gui 自包含、不依赖 towc.exe；与 tows 之间单 WS 多路复用，所有隧道只需 1 个心跳，cookie 保活全局一个（§1/§2.5/§3）
  - **tows 须具备包级路由能力**（按帧头 tunnel_id 路由到内网服务），支持多路复用 + 路径解析双模式（§1/§3）
...
- **已补充（2026-08-06）**：
  ...
  - **版本协商（2026-08-06）**：客户端建 WS 第一帧发 HELLO(0x00) → tows 回 HELLO_ACK(0x07)；发现旧 tows 打印警告（towc 降级路径解析 / towc_gui 退出）；**towc 统一走多路复用协议**（单隧道=1 tunnel_id），路径解析仅兼容旧客户端（§3.0/§3.1）
